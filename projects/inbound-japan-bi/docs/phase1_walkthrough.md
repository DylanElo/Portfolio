...
- **Fetches Data**: `etl/01_fetch_jnto_arrivals.py` checks for the official JNTO CSV. If missing (due to form requirements), it generates a realistic mock dataset (2019-2025) so you can test the pipeline immediately.
...
- **Features**:
    - **KPI Cards**: Total Visitors (latest year), Top Market, Recovery Rate.
    - **Trend Chart**: Monthly visitor volume showing the COVID dip and recovery.
    - **Market Chart**: Top 10 source markets for the latest year.
...
- **Data Integrity**:
    ...
    - `dim_month`: 84 rows.
    - `fact_inbound_arrivals_monthly`: 1680 rows (Mock data successfully loaded through 2025).
